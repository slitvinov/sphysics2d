...
c    along with this program.  If not, see <http://www.gnu.org/licenses/>.
      ...
      subroutine approx_RiemannSolver(i,j,drx,drz,rr2,cbar,robar,
...
c       dy_j = -(y_ij_av - yp(j))
        dz_j = -(z_ij_av - zp(j))
        delu_i = dudx_CSPHo(i)*dx_i !+ dudy_CSPHo(i)*dy_i
     &         + dudz_CSPHo(i)*dz_i
c       delv_i = dvdx_CSPHo(i)*dx_i  + dvdy_CSPHo(i)*dy_i
c    &         + dvdz_CSPHo(i)*dz_i
        delw_i = dwdx_CSPHo(i)*dx_i !+ dwdy_CSPHo(i)*dy_i
     &         + dwdz_CSPHo(i)*dz_i
        delrho_i = drhodx_CSPHo(i)*dx_i !+ drhody_CSPHo(i)*dy_i
     &           + drhodz_CSPHo(i)*dz_i
        delu_j = dudx_CSPHo(j)*dx_j !+ dudy_CSPHo(j)*dy_j
     &         + dudz_CSPHo(j)*dz_j
c       delv_j = dvdx_CSPHo(j)*dx_j  + dvdy_CSPHo(j)*dy_j
c    &         + dvdz_CSPHo(j)*dz_j
        delw_j = dwdx_CSPHo(j)*dx_j !+ dwdy_CSPHo(j)*dy_j
     &         + dwdz_CSPHo(j)*dz_j
        delrho_j = drhodx_CSPHo(j)*dx_j !+ drhody_CSPHo(j)*dy_j
     &           + drhodz_CSPHo(j)*dz_j
        delTE_i = dTEdx_CSPHo(i)*dx_i !+ dTEdy_CSPHo(i)*dy_i
     &          + dTEdz_CSPHo(i)*dz_i
        delTE_j = dTEdx_CSPHo(j)*dx_j !+ dTEdy_CSPHo(j)*dy_j
     &          + dTEdz_CSPHo(j)*dz_j
        d_rho_ij  = rhop(j) - rhop(i)
        d_u_ij    = up(j) - up(i)
c       d_v_ij    = vp(j) - vp(i)
        d_w_ij    = wp(j) - wp(i)
        d_TE_ij   = TEp(j) - TEp(i)
        !- MUSCL Limiters - Toro (2001) p. 208
        call limiter(i,j,delu_i,delu_j,d_u_ij,
     &                   del_u_lim_i,del_u_lim_j)
c       call limiter(i,j,delv_i,delv_j,d_v_ij,
c    &                   del_v_lim_i,del_v_lim_j)
        call limiter(i,j,delw_i,delw_j,d_w_ij,
     &                   del_w_lim_i,del_w_lim_j)
        call limiter(i,j,delTE_i,delTE_j,d_TE_ij,
     &                   del_TE_lim_i,del_TE_lim_j)
     ...
     &                   del_rho_lim_i,del_rho_lim_j)
        ...
        u_ij = up(i) + del_u_lim_i
        w_ij = wp(i) + del_w_lim_i

        u_ji = up(j) - del_u_lim_j
        w_ji = wp(j) - del_w_lim_j

        TE_ij = TEp(i) + del_TE_lim_i
        TE_ji = TEp(j) - del_TE_lim_j
      end if
      ...
      TE_L = TE_ij
      TE_R = TE_ji
      TotalE_L = rho_L*(TE_L + 0.5*(u_L*u_L + w_L*w_L))
      TotalE_R = rho_R*(TE_R + 0.5*(u_R*u_R + w_R*w_R))
      u_0 = 0.5*(u_ij + u_ji)
      w_0 = 0.5*(w_ij + w_ji)
...
c       rho_v_HLLC  = v_L   *Q_factor_L
        ...
        TotalE_HLLC = Q_factor_L*((TotalE_L/rho_L) + 
...
c       rho_v_HLLC  = v_R   *Q_factor_R
        ...
        TotalE_HLLC = Q_factor_R*((TotalE_R/rho_R) + 
...
800   format(a8,' ',e24.16)        
      ...
      TotalE_a =  TotalE_HLLC
      TE_a     =  TotalE_a/rho_a - 0.5*(u_a*u_a + w_a*w_a)
