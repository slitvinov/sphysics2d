...
c          -- Cells in the same XY sheet -
           lx2 = lx+1
           if(lx2.le.ncx)then
             call pre_celij_Shepard(j1,j1+1,kind_p1,ini_kind_p2,lx2)     !East
           endif      
           lz2=lz+1
           if(lz2.le.ncz)then       
             !- Same row -
             call pre_celij_Shepard(j1,j1+ncx,kind_p1,ini_kind_p2,lx2)   !North       
             lx2=lx+1
             if(lx2.gt.2) call pre_celij_Shepard(j1,j1+ncx-1,
     &                                 kind_p1,ini_kind_p2,lx2)   !North-West       
             lx2=lx+1
             if(lx2.le.ncx) call pre_celij_Shepard(j1,j1+ncx+1,
     ...
     +        call pre_self_Shepard(j1,kind_p1,ini_kind_p2)
       enddo
...
c      -- Periodic Boundary Calls in X-Direction --              
       if(i_periodicOBs(1).eq.1)then
         ...
         do lz = 1,ncz-1
           j1 = lx + (lz-1)*ncx
           call pre_celij_Shepard(j1,j1+ncall11,kind_p1,ini_kind_p2,lx2) !East X-Periodic
           call pre_celij_Shepard(j1,j1+ncall13,kind_p1,ini_kind_p2,lx2) !North-East X-Periodic
         ...
         do lz = 1,ncz-1
           j1 = lx + (lz-1)*ncx
           call pre_celij_Shepard(j1,j1+ncall12,kind_p1,ini_kind_p2,lx2) !North-West X-Periodic
         end do    !End of:  do lz = 1,ncz-1       
