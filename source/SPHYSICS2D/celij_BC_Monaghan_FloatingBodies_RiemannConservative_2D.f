...
c               az(i) = az(i) + temp*duz
               ...
               if(j.gt.nbfm)then  !Opposite Forces on free-moving object (Not per unit mass, see subroutine whole_body_motion)
                !Force on free-moving object  !i is free-moving WP, j is prescibed BP
                 ax(j) = ax(j) - fxbMON*Volrho(i)
                 az(j) = az(j) - fzbMON*Volrho(i)

                 !- Viscous wall force -
                 !ax(j) = ax(j) - temp*dux*pm(i)   !NOT ACTIVE YET
                 !az(j) = az(j) - temp*duz*pm(i)   !NOT ACTIVE YET
...
c               az(j) = az(j) - temp*duz
               ...
               if(i.gt.nbfm)then  !Opposite Forces on free-moving object (Not per unit mass, see subroutine whole_body_motion)
                 ...
                 !- Viscous wall force -
                 !ax(i) = ax(i) + temp*dux*pm(j)   !NOT ACTIVE YET
                 !az(i) = az(i) + temp*duz*pm(j)   !NOT ACTIVE YET
